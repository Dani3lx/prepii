...
export const generateFeedback = async (props: GenerateFeedbackParams): Promise<FeedbackGenerationResponse> => {
    ...
    try {
        ...
        const formattedQuestions = questions.map((question, index) => `Question ${index + 1}: ${question}\n`).join();
        ...
        try {
            ...
            const cached = ((await redisClient.json.get(cacheKey)) as FeedbackSummary[]) || null;
            if (cached) {
